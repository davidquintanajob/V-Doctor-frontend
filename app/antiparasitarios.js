...
export default function AntiparasitariosScreen() {
    ...
    // Cargar antiparasitarios desde la API al montar la pantalla
    useEffect(() => {
        (async () => {
            ...
            try {
                ...
                const url = `${host.replace(/\/+$/, '')}/venta/medicamentoPaciente/${id}/antiparasitario`;
                ...
                if (res.ok) {
                    ...
                    setAntiparasitariosList(Array.isArray(data) ? data : (data?.rows || []));
                } else {
                    console.warn('Error cargando antiparasitarios:', res.status);
                    setAntiparasitariosList([]);
                }
            } catch (e) {
                console.error('Error fetching antiparasitarios:', e);
                setAntiparasitariosList([]);
            } finally {
    ...
    return (
        <View style={styles.container}>
            ...
            <PatientSidebarMenu isOpen={showPatientMenu} onClose={() => setShowPatientMenu(false)} paciente={pacienteParam} apiHost={apiHost} selectedItem={'antiparasitarios'} />
        </View>
    );
