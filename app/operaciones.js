...
export default function OperacionesScreen() {
    ...
    // función reutilizable para cargar operaciones (ventas filtradas)
    const fetchOperaciones = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(url, {
                ...
                body: JSON.stringify({ tipo_comerciable: 'servicio_complejo', detallado: true })
            });

            if (!res.ok) {
                console.warn('Error cargando ventas/operaciones:', res.status);
                setOperacionesList([]);
                return;
            }
            ...
            const filtered = items.filter(it => {
                try {
                    ...
                    const isOperacion = typeof tipo === 'string' && tipo.toLowerCase().includes('oper');
            ...
            setOperacionesList(filtered);

        } catch (e) {
            console.error('Error fetching ventas/operaciones:', e);
            setOperacionesList([]);
        } finally {
                ...
                <View style={[styles.section, { minHeight: 200, marginTop: Spacing.m }]}>
                    <View style={{ marginBottom: Spacing.m }}>
                        ...
                        {!loadingOperaciones && operacionesList.map((c, idx) => {
                            ...
                            // nombre del producto/servicio si está disponible
                            const productoNombre = c?.comerciable?.servicio?.descripcion || c?.comerciable?.producto?.nombre || ('Venta ' + (c.id_venta ?? c.id ?? idx));
