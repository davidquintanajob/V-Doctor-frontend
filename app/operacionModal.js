...
export default function OperacionModalScreen() {
    ...
    const [consultaData, setConsultaData] = useState({
        ...
        fecha: consultaParam?.fecha ? formatDateLocal(consultaParam.fecha) : formatDateLocal(new Date()),
    ...
    const [apiAutocompleteInitial, setApiAutocompleteInitial] = useState(null);
    // Selección actual del ApiAutocomplete (servicio complejo elegido)
    const [apiAutocompleteSelected, setApiAutocompleteSelected] = useState(null);
    // Precio original y precio cobrado del servicio complejo
    // `apiAutocompleteOriginalPrice` se edita manualmente por el usuario y representa
    // `precio_original_comerciable_cup` que será enviado como precio original.
    // `apiAutocompleteCobradoPrice` es el precio final cobrado (precio_cobrado_cup)
    // y se modifica sólo al aplicar un delta desde TotalsEditModal.
    const [apiAutocompleteOriginalPrice, setApiAutocompleteOriginalPrice] = useState('');
    const [apiAutocompleteCobradoPrice, setApiAutocompleteCobradoPrice] = useState('');
    const [apiCobradoManuallyEdited, setApiCobradoManuallyEdited] = useState(false);

    // Mantener la selección del autocomplete en sync con el initial (cuando se carga desde ventas existentes)
    React.useEffect(() => {
        if (apiAutocompleteInitial) {
            setApiAutocompleteSelected(apiAutocompleteInitial);
            // Inicializar precio original y cobrado según los datos pasados
            const originalVal = apiAutocompleteInitial?.precio_original_comerciable_cup ?? apiAutocompleteInitial?.servicio?.comerciable?.precio_cup ?? '';
            const cobradoVal = apiAutocompleteInitial?.precio_cobrado_cup ?? originalVal;
            setApiAutocompleteOriginalPrice(originalVal != null ? String(originalVal) : '');
            setApiAutocompleteCobradoPrice(cobradoVal != null ? String(cobradoVal) : '');
            setApiCobradoManuallyEdited(false);
        } else {
            setApiAutocompleteSelected(null);
            setApiAutocompleteOriginalPrice('');
            setApiAutocompleteCobradoPrice('');
        }
    }, [apiAutocompleteInitial]);

    // Mantener el precio cobrado en sync con el precio original salvo que el cobrado haya sido editado
    useEffect(() => {
        try {
            if (!apiCobradoManuallyEdited) {
                // Si no se ha editado manualmente el cobrado, sincronizarlo al cambiar el original
                setApiAutocompleteCobradoPrice(apiAutocompleteOriginalPrice || '');
            }
        } catch (e) { /* ignore */ }
    }, [apiAutocompleteOriginalPrice, apiCobradoManuallyEdited]);
    ...
    React.useEffect(() => {
        ...
        const buildTratamientoParts = () => {
            try {
                const refs = [medicamentosListRef, productosListRef];
                const parts = [];
    ...
    }, [mode, medicamentosListRef, productosListRef]);
        ...
        const procesarVentasExistentes = () => {
            ...
            ventas.forEach((venta, index) => {
                ...
                if (comerciable.servicio) {
                    // Detectar si este servicio es un servicio complejo
                    const svc = comerciable.servicio;
                    
                    const servicioComplejoObj = svc?.servicio_complejo || venta?.servicio_complejo || null;

                    if (servicioComplejoObj && !complexServiceFound) {
                        // Guardar el servicio complejo encontrado para inicializar el ApiAutocomplete
                        // Incluimos el precio cobrado en la venta si existe para mostrarlo correctamente
                        complexServiceFound = {
                            servicio: svc,
                            tipo_servicio: servicioComplejoObj?.tipo_servicio || null,
                            servicio_complejo: servicioComplejoObj,
                            // Precios provenientes de la venta (si se abrió el modal con ventas)
                            precio_cobrado_cup: venta?.precio_cobrado_cup ?? null,
                            precio_original_comerciable_cup: venta?.precio_original_comerciable_cup ?? null,
                            costo_producto_cup: venta?.costo_producto_cup ?? null
                        };
                        // No agregar a la lista de servicios regulares
                    } else {
                        // Es un servicio normal (o ya encontramos el complejo y este es adicional): agregar a lista de servicios regulares
                        serviciosItems.push({
                            id: id,
            ...
            // Fusionar vacunas y antiparasitarios dentro de la lista de medicamentos
            setInitialMedicamentos([...(medicamentosItems || []), ...(vacunasItems || []), ...(antiparasitariosItems || [])]);
            // Mantener arrays separados vacíos/por compatibilidad (no se muestran en esta vista)
            setInitialVacunas([]);
            setInitialAntiparasitarios([]);
            // Si encontramos un servicio complejo en las ventas, exponerlo como initial para ApiAutocomplete
            if (complexServiceFound) {
                setApiAutocompleteInitial(complexServiceFound);
            } else {
                setApiAutocompleteInitial(null);
            }
    ...
    const calculateTotals = () => {
        // Si hay un servicio complejo seleccionado por el ApiAutocomplete, tratarlo como única venta
            if (apiAutocompleteSelected) {
            const price = parseFloat(apiAutocompleteCobradoPrice || apiAutocompleteSelected?.precio_cobrado_cup || apiAutocompleteOriginalPrice || 0) || 0;
            // Intentar obtener costo desde la venta (precio original / costo) si fue pasado por params,
            // si no, usar el precio del comerciable como costo de referencia
            const costo = parseFloat(
                apiAutocompleteSelected?.costo_producto_cup ??
                apiAutocompleteSelected?.precio_original_comerciable_cup ??
                apiAutocompleteSelected?.servicio?.comerciable?.costo_producto_cup ??
                apiAutocompleteSelected?.servicio?.comerciable?.precio_cup ??
                0
            ) || 0;
            const finalTotalCobrar = isNaN(price) ? 0 : price;
            const finalTotalProfit = isNaN(price - costo) ? 0 : Math.max(0, price - costo);

            // Preparar la estructura originalTotalsRef exactamente igual que en el flujo normal
        ...
        const refs = [medicamentosListRef, productosListRef, serviciosListRef];
    ...
    const createVentasForList = async (cfgHost, cfgToken, id_consulta, itemsList, tipo, usuariosIds, relatedVentaId = null) => {
        ...
        for (let i = 0; i < itemsList.length; i++) {
            ...
            if (tipo === 'servicios') {
                ...
                body = {
                    ...
                    precio_cobrado_cup: 0,
                    forma_pago: (paymentType === 'efectivo') ? 'Efectivo' : 'Transferencia',
                    id_consulta: id_consulta,
                    id_comerciable: parseInt(comerciable.id_comerciable || comerciable.id || 0, 10) || 0,
                    id_usuario: usuariosIds || [],
                };
                // Incluir referencia a venta relacionada si aplica
                if (relatedVentaId) body.id_venta_relacionada = relatedVentaId;
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            // Requerir que haya un servicio complejo seleccionado en el ApiAutocomplete
            if (!apiAutocompleteSelected) {
                Alert.alert('Atención', 'Debe seleccionar un servicio complejo antes de continuar.');
                setLoading(false);
                return;
            }

            // Validar la venta del elemento seleccionado (servicio complejo)
            const usuariosSeleccionados_for_validation = usuariosListRef.current && typeof usuariosListRef.current.getItems === 'function'
                ? (usuariosListRef.current.getItems() || [])
                : [];
            const usuariosIds_for_validation = usuariosSeleccionados_for_validation.map(u => u.id_usuario || u.id).filter(Boolean);
            const validateSelectedBody = {
                fecha: new Date(consultaData.fecha).toISOString(),
                precio_original_comerciable_cup: parseFloat(apiAutocompleteSelected?.servicio?.comerciable?.precio_cup || 0) || 0,
                precio_original_comerciable_usd: parseFloat(apiAutocompleteSelected?.servicio?.comerciable?.precio_usd || 0) || 0,
                costo_producto_cup: 0,
                cantidad: 1,
                precio_cobrado_cup: parseFloat(apiAutocompleteCobradoPrice || apiAutocompleteSelected?.precio_cobrado_cup || apiAutocompleteOriginalPrice || apiAutocompleteSelected?.servicio?.comerciable?.precio_cup || 0) || 0,
                forma_pago: (paymentType === 'efectivo') ? 'Efectivo' : 'Transferencia',
                id_comerciable: parseInt(apiAutocompleteSelected?.servicio?.comerciable?.id_comerciable || apiAutocompleteSelected?.servicio?.comerciable?.id || 0, 10) || 0,
                id_usuario: usuariosIds_for_validation || [],
            };
            setValidationVisible(true);
            setValidationTitle('Validando servicio seleccionado...');
            const urlValidateSingle = `${host.replace(/\/+$/, '')}/venta/validate`;
            const headersValidateSingle = { 'Content-Type': 'application/json' };
            if (token) headersValidateSingle['Authorization'] = `Bearer ${token}`;
            const resValidateSingle = await fetch(urlValidateSingle, { method: 'POST', headers: headersValidateSingle, body: JSON.stringify(validateSelectedBody) });
            let respValidateSingleData = null;
            try { respValidateSingleData = await resValidateSingle.json(); } catch (e) { respValidateSingleData = null; }
            if (!resValidateSingle.ok) {
                let errorMessage = 'Error validando servicio seleccionado';
                if (respValidateSingleData && respValidateSingleData.errors && Array.isArray(respValidateSingleData.errors)) {
                    errorMessage = respValidateSingleData.errors.join('\n• ');
                } else if (respValidateSingleData && (respValidateSingleData.message || respValidateSingleData.description)) {
                    errorMessage = respValidateSingleData.message || respValidateSingleData.description;
                }
            ...
            // Primero validar Medicamentos (incluye vacunas y antiparasitarios fusionados)
            ...
            try {
                ...
                if (!idConsultaCreada) {
                    ...
                    // Crear primero la venta del servicio complejo seleccionado y obtener su id
                    let relatedVentaId = null;
                    try {
                        setValidationTitle('Creando venta del servicio seleccionado...');
                        const seleccion = apiAutocompleteSelected || {};
                        const bodySelected = {
                            fecha: new Date(consultaData.fecha).toISOString(),
                            precio_original_comerciable_cup: parseFloat(apiAutocompleteOriginalPrice || seleccion?.precio_original_comerciable_cup || seleccion?.servicio?.comerciable?.precio_cup || 0) || 0,
                            precio_original_comerciable_usd: parseFloat(seleccion?.servicio?.comerciable?.precio_usd || 0) || 0,
                            costo_producto_cup: 0,
                            cantidad: 1,
                            nota: seleccion?.nota || '',
                            exedente_redondeo: 0,
                            descuento: pacienteParam?.descuento || 0,
                            precio_cobrado_cup: parseFloat(apiAutocompleteCobradoPrice || seleccion?.precio_cobrado_cup || apiAutocompleteOriginalPrice || seleccion?.servicio?.comerciable?.precio_cup || 0) || 0,
                            forma_pago: (paymentType === 'efectivo') ? 'Efectivo' : 'Transferencia',
                            id_consulta: idConsultaCreada,
                            id_comerciable: parseInt(seleccion?.servicio?.comerciable?.id_comerciable || seleccion?.servicio?.comerciable?.id || 0, 10) || 0,
                            id_usuario: usuariosIds || [],
                        };

                        const urlCreateVentaSelected = `${host.replace(/\/+$/, '')}/venta/create`;
                        const headersCreateVenta = { 'Content-Type': 'application/json' };
                        if (token) headersCreateVenta['Authorization'] = `Bearer ${token}`;
                        const resCreateVenta = await fetch(urlCreateVentaSelected, { method: 'POST', headers: headersCreateVenta, body: JSON.stringify(bodySelected) });
                        let responseCreateVentaData = null;
                        try { responseCreateVentaData = await resCreateVenta.json(); } catch (e) { responseCreateVentaData = null; }
                        if (!resCreateVenta.ok) {
                            let errorMessage = 'Error creando venta del servicio seleccionado';
                            if (responseCreateVentaData && responseCreateVentaData.errors && Array.isArray(responseCreateVentaData.errors)) {
                                errorMessage = responseCreateVentaData.errors.join('\n• ');
                            } else if (responseCreateVentaData && (responseCreateVentaData.message || responseCreateVentaData.description)) {
                                errorMessage = responseCreateVentaData.message || responseCreateVentaData.description;
                            }
                            Alert.alert('Error', errorMessage);
                            setValidationVisible(false);
                            return;
                        ...
                        // Extraer id de la venta creada
                        if (responseCreateVentaData && responseCreateVentaData.data && responseCreateVentaData.data.id_venta) {
                            relatedVentaId = responseCreateVentaData.data.id_venta;
                        } else if (responseCreateVentaData && responseCreateVentaData.id_venta) {
                            relatedVentaId = responseCreateVentaData.id_venta;
                        } else if (responseCreateVentaData && responseCreateVentaData.venta && responseCreateVentaData.venta.id_venta) {
                            relatedVentaId = responseCreateVentaData.venta.id_venta;
                        }

                        if (!relatedVentaId) {
                            // No se pudo obtener id — continuar pero sin relacionar
                            console.warn('No se obtuvo id de la venta creada para el servicio seleccionado');
                            relatedVentaId = null;
                        }
                    } catch (errCreateSel) {
                        console.error('Error creando venta seleccionada:', errCreateSel);
                        Alert.alert('Error', 'No se pudo crear la venta del servicio seleccionado');
                        setValidationVisible(false);
                        return;
                    }
                ...
                // Calcular el total de ventas a crear (medicamentos incluye vacunas y antiparasitarios)
                const totalVentas = medicamentosFiltrados.length + serviciosFiltrados.length + productosFiltrados.length;
                setTotalVentasToCreate(totalVentas);
                setVentasCreated(0);

                if (totalVentas > 0) {
                    // Crear ventas de medicamentos (incluye vacunas/antiparasitarios), servicios y productos
                        if (medicamentosFiltrados.length > 0) {
                        const resultMed = await createVentasForList(host, token, idConsultaCreada, medicamentosFiltrados, 'medicamentos', usuariosIds, relatedVentaId);
                        if (!resultMed.ok) {
                            setValidationVisible(false);
                            return;
                        }
                    ...
                    if (serviciosFiltrados.length > 0) {
                        const resultServ = await createVentasForList(host, token, idConsultaCreada, serviciosFiltrados, 'servicios', usuariosIds, relatedVentaId);
                        if (!resultServ.ok) {
                            setValidationVisible(false);
                            return;
                        }
                    ...
                    if (productosFiltrados.length > 0) {
                        const resultProd = await createVentasForList(host, token, idConsultaCreada, productosFiltrados, 'productos', usuariosIds, relatedVentaId);
                        if (!resultProd.ok) {
                            setValidationVisible(false);
                            return;
                        }
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            const productosItems = (productosListRef.current && typeof productosListRef.current.getItems === 'function')
                ? (productosListRef.current.getItems() || [])
                : [];
            // Para actualizar la consulta: validar que exista un servicio complejo seleccionado
            if (!apiAutocompleteSelected) {
                Alert.alert('Atención', 'Debe seleccionar un servicio complejo antes de continuar.');
                setLoading(false);
                return;
            ...
            // Validar la venta del elemento seleccionado en el ApiAutocomplete
            try {
                const usuariosSeleccionados_for_validation = usuariosListRef.current && typeof usuariosListRef.current.getItems === 'function'
                    ? (usuariosListRef.current.getItems() || [])
                    : [];
                ...
                const validateSelectedBody = {
                fecha: new Date(consultaData.fecha).toISOString(),
                precio_original_comerciable_cup: parseFloat(apiAutocompleteOriginalPrice || apiAutocompleteSelected?.precio_original_comerciable_cup || apiAutocompleteSelected?.servicio?.comerciable?.precio_cup || 0) || 0,
                precio_original_comerciable_usd: parseFloat(apiAutocompleteSelected?.servicio?.comerciable?.precio_usd || 0) || 0,
                costo_producto_cup: 0,
                cantidad: 1,
                precio_cobrado_cup: parseFloat(apiAutocompleteCobradoPrice || apiAutocompleteSelected?.precio_cobrado_cup || apiAutocompleteOriginalPrice || apiAutocompleteSelected?.servicio?.comerciable?.precio_cup || 0) || 0,
                ...
                setValidationVisible(true);
                setValidationTitle('Validando servicio seleccionado...');
                const urlValidate = `${host.replace(/\/+$/, '')}/venta/validate`;
                const headersValidate = { 'Content-Type': 'application/json' };
                if (token) headersValidate['Authorization'] = `Bearer ${token}`;
                const resValidate = await fetch(urlValidate, { method: 'POST', headers: headersValidate, body: JSON.stringify(validateSelectedBody) });
                let respValidateData = null;
                try { respValidateData = await resValidate.json(); } catch (e) { respValidateData = null; }
                if (!resValidate.ok) {
                    let errorMessage = 'Error validando servicio seleccionado';
                    if (respValidateData && respValidateData.errors && Array.isArray(respValidateData.errors)) {
                        errorMessage = respValidateData.errors.join('\n• ');
                    } else if (respValidateData && (respValidateData.message || respValidateData.description)) {
                        errorMessage = respValidateData.message || respValidateData.description;
                    }
                    Alert.alert('Error', errorMessage);
                    setValidationVisible(false);
                    setLoading(false);
                ...
                // Actualizar la consulta (sin ventas) primero
                ...
                setValidationTitle('Actualizando consulta...');
                setValidationProgress(96);
                ...
                const resUpdate = await fetch(urlUpdate, { method: 'PUT', headers: headersUpdate, body: JSON.stringify(payloadConsulta) });
                let resUpdateData = null;
                try { resUpdateData = await resUpdate.json(); } catch (e) { resUpdateData = null; }
                if (!resUpdate.ok) {
                    let errorMessage = 'Error actualizando consulta';
                    if (resUpdateData && resUpdateData.errors && Array.isArray(resUpdateData.errors)) {
                        errorMessage = resUpdateData.errors.join('\n• ');
                    } else if (resUpdateData && (resUpdateData.message || resUpdateData.description)) {
                        errorMessage = resUpdateData.message || resUpdateData.description;
                    }
                    Alert.alert(`Error ${resUpdate.status}`, errorMessage);
                    setValidationVisible(false);
                    return;
                }

                // Crear la venta del servicio seleccionado y obtener su id
                let relatedVentaId = null;
                try {
                    setValidationTitle('Creando venta del servicio seleccionado...');
                    const seleccion = apiAutocompleteSelected || {};
                    const bodySelected = {
                        fecha: new Date(consultaData.fecha).toISOString(),
                        precio_original_comerciable_cup: parseFloat(seleccion?.servicio?.comerciable?.precio_cup || 0) || 0,
                        precio_original_comerciable_usd: parseFloat(seleccion?.servicio?.comerciable?.precio_usd || 0) || 0,
                        costo_producto_cup: 0,
                        cantidad: 1,
                        nota: seleccion?.nota || '',
                        ...
                        precio_cobrado_cup: parseFloat(apiAutocompleteCobradoPrice || seleccion?.precio_cobrado_cup || apiAutocompleteOriginalPrice || seleccion?.servicio?.comerciable?.precio_cup || 0) || 0,
                        forma_pago: (paymentType === 'efectivo') ? 'Efectivo' : 'Transferencia',
                        id_consulta: consultaData.id_consulta || consultaParam?.id_consulta || consultaParam?.id,
                        id_comerciable: parseInt(seleccion?.servicio?.comerciable?.id_comerciable || seleccion?.servicio?.comerciable?.id || 0, 10) || 0,
                        id_usuario: usuariosIds || [],
                    };
                ...
                // Ahora crear las ventas para todas las listas, pasando relatedVentaId
                setValidationProgress(96);
                const medicamentosFiltrados = medicamentosItems.filter(item => item.selected);
                const serviciosFiltrados = serviciosItems.filter(item => item.selected);
                const productosFiltrados = productosItems.filter(item => item.selected);
                ...
                if (totalVentas > 0) {
                    if (medicamentosFiltrados.length > 0) {
                        const resultMed = await createVentasForList(host, token, consultaData.id_consulta || consultaParam?.id_consulta || consultaParam?.id, medicamentosFiltrados, 'medicamentos', usuariosIds, relatedVentaId);
                        if (!resultMed.ok) {
                            setValidationVisible(false);
                            return;
                        }
                    ...
                    if (serviciosFiltrados.length > 0) {
                        const resultServ = await createVentasForList(host, token, consultaData.id_consulta || consultaParam?.id_consulta || consultaParam?.id, serviciosFiltrados, 'servicios', usuariosIds, relatedVentaId);
                        if (!resultServ.ok) {
                            setValidationVisible(false);
                            return;
                        }
                    ...
                    if (productosFiltrados.length > 0) {
                        const resultProd = await createVentasForList(host, token, consultaData.id_consulta || consultaParam?.id_consulta || consultaParam?.id, productosFiltrados, 'productos', usuariosIds, relatedVentaId);
                        if (!resultProd.ok) {
                            setValidationVisible(false);
                            return;
                        }
                ...
                setValidationProgress(100);
                setValidationTitle('Consulta actualizada y ventas creadas correctamente');
                setTimeout(() => {
                    setValidationVisible(false);
                    ToastAndroid.show('Consulta actualizada con éxito', ToastAndroid.LONG);
                }, 800);

            } catch (errUpdate) {
                console.error('Error en handleUpdate flujo:', errUpdate);
                Alert.alert('Error', errUpdate.message || 'Error desconocido en actualización');
                setValidationVisible(false);
                return;
            }
    ...
    const TotalsEditModal = () => {
        ...
        const onSave = () => {
            // Sumar el delta definido en el modal al precio cobrado final del servicio seleccionado
            const delta = parseFloat((localDelta || '').toString().replace(/,/g, '.')) || 0;
            const curr = parseFloat((apiAutocompleteCobradoPrice || apiAutocompleteOriginalPrice || '').toString().replace(/,/g, '.')) || 0;
            const next = curr + delta;
            setApiAutocompleteCobradoPrice(String(Number(next).toFixed(2)));
            setApiCobradoManuallyEdited(true);
            // Resetear el delta local y cerrar modal
            setLocalDelta('0.00');
            setTotalsModalVisible(false);
        };

        const onReset = () => {
            // Restaurar el precio original del elemento seleccionado en el ApiAutocomplete
            const originalPrice = parseFloat(
                apiAutocompleteSelected?.precio_original_comerciable_cup ??
                apiAutocompleteSelected?.servicio?.comerciable?.precio_cup ??
                0
            ) || 0;
            setApiAutocompleteOriginalPrice(String(Number(originalPrice).toFixed(2)));
            // Asegurar que el precio cobrado vuelva a ser igual al original
            setApiAutocompleteCobradoPrice(String(Number(originalPrice).toFixed(2)));
            setApiCobradoManuallyEdited(false);
            // También resetear campos locales del modal
            const origTotals = originalTotalsRef.current || { totalCobrar: 0, totalConDescuento: 0, version: 0 };
            setUserTotals({ totalCobrar: origTotals.totalCobrar, totalConDescuento: origTotals.totalConDescuento, version: origTotals.version });
            setLocalTotalCobrar(String(Number(origTotals.totalCobrar).toFixed(2)));
            setLocalTotalConDescuento(String(Number(origTotals.totalConDescuento).toFixed(2)));
            setLocalDelta('0.00');
        };
        ...
        >
            <View style={styles.container}>
                ...
                <ScrollView ref={scrollRef} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="always">
                    ...
                    {/* Selector de Servicio Complejo (API) + campos tipo_servicio y precio */}
                    <View style={styles.section}>
                        <Text style={[styles.label, { marginBottom: Spacing.xs }]}>
                            {(mode === 'ver' || mode === 'editar') ? 'Detalles del servicio complejo' : 'Seleccione un servicio complejo'}
                        </Text>
                        <ApiAutocomplete
                            endpoint="/servicioComplejo/filter/5/1"
                            searchKey="descripcion"
                            style={[ styles.apiAutocompletable ]}
                            placeholder="Buscar servicio complejo..."
                            displayFormat={(item) => `${item.servicio?.descripcion || ''} - ${item.tipo_servicio || ''} - $ ${item.servicio?.comerciable?.precio_cup || ''}`}
                            onItemSelect={(item) => {
                                // Actualizar la selección y popular los precios iniciales
                                setApiAutocompleteSelected(item || null);
                                const original = item?.precio_original_comerciable_cup ?? item?.servicio?.comerciable?.precio_cup ?? '';
                                const cobrado = item?.precio_cobrado_cup ?? original;
                                setApiAutocompleteOriginalPrice(original != null ? String(original) : '');
                                setApiAutocompleteCobradoPrice(cobrado != null ? String(cobrado) : '');
                                setApiCobradoManuallyEdited(false);
                            }}
                            initialValue={apiAutocompleteInitial}
                        />
                        ...
                        <View style={{ flexDirection: 'row', gap: Spacing.s, marginTop: Spacing.s }}>
                            <View style={{ flex: 1 }}>
                                ...
                                <TextInput
                                    style={[styles.input, { flex: 1, opacity: 0.5 }]}
                                    placeholder="Tipo de servicio"
                                    value={apiAutocompleteSelected?.tipo_servicio || ''}
                                    editable={false}
                                />
                            </View>
                            ...
                            <View style={{ flex: 1 }}>
                                ...
                                <TextInput
                                    ...
                                    value={apiAutocompleteOriginalPrice}
                                    onChangeText={(t) => setApiAutocompleteOriginalPrice(t)}
                                    keyboardType="numeric"
                                    editable={isEditable}
                                />
                            </View>
                    ...
                    {/* Lista de Medicamentos (vacunas/antiparasitarios fusionados aquí) */}
                    <MedicamentosLista
                        ...
                        return (
                            <View style={styles.section}>
                                ...
                                <View style={styles.summaryRow}>
                                    ...
                                    {(() => {
                                        // Si hay un servicio complejo seleccionado, el plus se calcula
                                        // como (precio mostrado/definido por el usuario) - precio_original_comerciable_cup
                                        if (apiAutocompleteSelected) {
                                            const originalSource = apiAutocompleteOriginalPrice || (apiAutocompleteSelected?.precio_original_comerciable_cup ?? apiAutocompleteSelected?.servicio?.comerciable?.precio_cup ?? 0);
                                            const original = parseFloat(originalSource) || 0;
                                            const current = parseFloat(apiAutocompleteCobradoPrice || apiAutocompleteSelected?.precio_cobrado_cup || original || 0) || 0;
                                            let plusValue = current - original;
                                        ...
                                        // Sin servicio seleccionado: mantener la lógica previa
